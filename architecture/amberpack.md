...
files and the remote-sync wire packs that move objects between a daemon and a
`serve` instance. The shared unit is the **record** — a self-describing,
CRC-protected, individually-zstd-compressed CAS object. This document specifies
the record and the wire pack that frames a set of them. The on-disk segment
format (which wraps the same records in a header and a self-indexing footer)
lives in [packstore's package doc](../packstore/segment.go); see also
[remote.md](remote.md) for how packs travel over HTTP.
...
root key** — like a git pack. It is what `POST /v1/objects` and
`POST /v1/objects/get` carry (see [remote.md](remote.md)). Layout:
...
The codec exposes two `errors.Is` sentinels, mapped to client errors at the HTTP
layer (a malformed pack or a hash mismatch is a `422`; see the status table in
[remote.md](remote.md)):
